...
A clean Flask image watermarking app ready for direct GitHub/Portainer deployments from https://github.com/sefaction/Watermarker.
...
The default `docker-compose.yml` is intended for Portainer and builds directly from the GitHub repository:

```yaml
build:
  context: https://github.com/sefaction/Watermarker.git#main
```

Run the same GitHub-backed build locally with:
...
### Build locally from your current checkout

Use the local build file when developing unpushed changes:
...
1. Push this repository to GitHub at <https://github.com/sefaction/Watermarker>.
2. In Portainer, create a stack from a Git repository.
3. Point Portainer at `https://github.com/sefaction/Watermarker` and use `docker-compose.yml` as the compose path.
4. Deploy the stack. The compose file builds directly from `https://github.com/sefaction/Watermarker.git#main`, exposes container port `8000`, and maps it to host port `8000` by default.
5. Adjust the runtime environment variables in `docker-compose.yml` or Portainer if needed.

No manual file copying, GHCR package, or separate image registry is required; Portainer/Docker builds from the GitHub repository and generated job files persist in the Docker volume.
